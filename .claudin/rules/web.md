...
- Each page's JS defines a single `app()` factory returning the Alpine state object; `<body x-data="app()" x-init="init()">`. Keep all state on that object; `init()` starts with `await checkAuth()` and then loads data. (Exception: `stats.js` uses `statsApp()` — when editing an existing page, match its own pattern rather than renaming.)
...
- Tables/lists: `<template x-for>` (see `clients.html:195-246`). Forms are inline `x-show` panels, not modals; errors surface via `alert()` or inline messages — match the page you're editing. (Exception: `queries.html:220-221` has an `x-ref="modal"` details modal — legacy pattern; extend it there but don't replicate it elsewhere.)
