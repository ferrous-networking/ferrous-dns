...
- [x] DNSSEC anti-downgrade — an empty DS answer must come with the parent's authenticated NSEC/NSEC3 denial (RFC 4035 §5.2) before a delegation is treated as insecure; fails open when the upstream strips the authority section, counted at `GET /api/dnssec/stats`
