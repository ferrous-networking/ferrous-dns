...
At **477,744 queries/second** under identical Docker conditions (16 CPUs, cache enabled, log info, rate limiting disabled), Ferrous-DNS is **4.7× faster than AdGuard Home**, **4.7× faster than Blocky**, and **125× faster than Pi-hole** — all running a full feature stack (DNS server, REST API, Web UI, SQLite query log, blocking engine) in a single process. PowerDNS Recursor (899K QPS) and Unbound (1.1M QPS) lead as purpose-built pure recursive resolvers with no additional features.
